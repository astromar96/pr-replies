...
```
node "{{ROOT}}/server/server.js" suggest --repo $REPO --repo-dir $REPO_DIR
```
...
`{{ROOT}}/examples/payload.triage.json` (`version: 2`, repo, pr
...
For **GitLab**, follow `{{ROOT}}/examples/payload.triage.gitlab.json`
...
```
node "{{ROOT}}/server/server.js" serve --session $SESSION --repo-dir $REPO_DIR
```
...
```
node "{{ROOT}}/server/server.js" wait --session $SESSION --phase triage --timeout-secs 540
```
...
```
node "{{ROOT}}/server/server.js" wait --session $SESSION --phase reply --timeout-secs 540
```
...
```
node "{{ROOT}}/server/server.js" stop --session $SESSION
```
