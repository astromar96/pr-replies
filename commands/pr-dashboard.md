...
Launch in the **background** (run_in_background) so it lives after this turn.
Pass `--repo-dir "$PWD"` so the Templates view merges this repo's local
templates (`.pr-replies/templates.json`) on top of the user's:
